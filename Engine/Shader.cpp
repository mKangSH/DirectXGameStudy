...
void Shader::Init(const wstring& path, ShaderInfo info, const string& vs, const string& ps)
{
	...
	switch (info.shaderType)
	{
		...
		case SHADER_TYPE::LIGHTING:
			_pipelineDesc.NumRenderTargets = RENDER_TARGET_LIGHTING_GROUP_MEMBER_COUNT;
			_pipelineDesc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;
	...
	D3D12_RENDER_TARGET_BLEND_DESC& rtBlendState = _pipelineDesc.BlendState.RenderTarget[0];
	...
	switch (info.blendType)
	{
		...
		case BLEND_TYPE::ALPHA_BLEND:
			...
			rtBlendState.DestBlend = D3D12_BLEND_INV_DEST_ALPHA; // (1 - alpha)
			break;
