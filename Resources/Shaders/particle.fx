...
VS_OUT VS_Main(VS_IN input)
{
	...
	output.viewPos = mul(float4(worldPos, 1.0f), g_matView);
	output.uv = input.uv;
	output.id = input.id;
...
void GS_Main(point VS_OUT input[1], inout TriangleStream<GS_OUT> outputStream)
{
	...
	float scale = ((g_float_1 - g_float_0) * ratio + g_float_0) / 2.0f;

	output[0].position = vtx.viewPos + float4(-scale, scale, 0.0f, 0.0f);
	output[1].position = vtx.viewPos + float4(scale, scale, 0.0f, 0.0f);
	output[2].position = vtx.viewPos + float4(scale, -scale, 0.0f, 0.0f);
	output[3].position = vtx.viewPos + float4(-scale, -scale, 0.0f, 0.0f);
...
RWStructuredBuffer<ComputeShared> g_shared : register(u1); // 크기가 1인 배열 공용 데이터 저장

// 동일한 스레드 그룹으로 묶으면
// 동기화 기법과 메모리를 공유하여 접근이 가능하다.

// TODO : 아래 매개변수를 머티리얼에 적용하여 디스패치 Group 크기 (1, 1, 1)
// g_vec2_1 : DeltaTime | AccTime(Accumulate)
...
{
	// 병렬실행 시 실행 순서로 인한 보장되지 않은 동작을 예방
