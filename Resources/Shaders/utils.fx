...
LightComponents CalculateLightColor(int lightIndex, float3 viewNormal, float3 viewPos)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (centerDist < 0.f || centerDist > g_lights[lightIndex].range) // 최대 거리를 벗어났는지
            {
                distanceRatio = 0.f;
            }
...
float CalculateTessLevel(float3 cameraWorldPos, float3 patchPos, float min, float max, float maxLevel)
{
    ...
    float level = (maxLevel - 1.0f) * (1.0f - ratio);
